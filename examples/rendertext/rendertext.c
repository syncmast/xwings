...
int main(int argc, char **argv)
{
	...
	GlyphSet font=load_glyphset(display,
		"/usr/share/fonts/truetype/ttf-bitstream-vera/VeraBI.ttf", 24);
	
	XMapWindow(display, window);
	XRenderColor bg_color;
	bg_color.red=0xffff; bg_color.green=0xffff; bg_color.blue=0xffff; bg_color.alpha=0xffff;
		...
		switch(event.type) {
			case Expose:
				...
				XRenderCompositeString8(display, PictOpOver,
					fg_pen, picture, 0,
					font, 0, 0, 200, 240, "Hello World!", 12);
				break;
			case DestroyNotify:
				return 0;
		}
