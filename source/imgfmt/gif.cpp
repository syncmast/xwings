...
{
//	return (void *)new unsigned char[bytes];
//	return malloc(bytes);
...
//		data->bytes = app_zero_alloc(size * sizeof(unsigned char));
		data->bytes = (unsigned char *)calloc(size, sizeof(unsigned char));
...
static Colour rgb(unsigned char r, unsigned char g, unsigned char b)
{
	Colour ret;
	ret.red = r;
	ret.green = g;
...
//	cmap->colours = new Colour[cmap->length];
...
//	pic->data = new unsigned char *[h];
	if (pic->data == NULL)
		return;
	for (row=0; row < h; row++)
		pic->data[row] = (unsigned char *)calloc(w, sizeof(unsigned char));
		//pic->data[row] = new unsigned char[w];
